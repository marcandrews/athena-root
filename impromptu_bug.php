...
if ($_POST) {
	...
	if ($assoc['query'] = mysql_query($assoc['sql'], $info['sql_db_connect']) and mysql_num_rows($assoc['query']) == 1) {
...
//<![CDATA[
	...
	function form_reset() {
		$.prompt('Are you sure you want to clear the form?',{ container: '#wrapper', buttons: { Yes: true, No: false }, callback: mycallbackfunc });
		return false;
	}
//]]>
</script>
...
<?php
		...
		if ( $assoc['query'] = mysql_query($assoc['sql'], $info['sql_db_connect']) and mysql_num_rows($assoc['query']) > 0 ) {
			$assoc['results'] = mysql_fetch_assoc($assoc['query']);
?>
...
<?php
			do {
?>
						<option value="<?php print $assoc['results']['id'] ?>"<?php print $selected['to'][$assoc['results']['id']] ?>><?php print $assoc['results']['name'] ?></option>
<?php
