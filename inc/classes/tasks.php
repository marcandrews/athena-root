		...
		if ($task['query'] = mysql_query($task['sql']) and mysql_num_rows($task['query']) == 1) {
			...
			if ($task['results']['locked'] != 0) {
				...
				if (time() - strtotime($task['results']['locked']) > 1800) {
					...
					$this->set_next_runtime($task['results']['id']);
					$this->unlock_task($task['results']['id']);
					$this->log_task($task['results']['id'], "Task was locked for longer than 30 minutes.", true);
				}
			} else {
				// This task is not locked; run it and lock the it while it runs.
				$this->lock_task($task['results']['id']);
		
				// Run the task.
				$task_file = $_SERVER['DOCUMENT_ROOT']."inc/tasks/{$task['results']['file']}";
				if (is_file($task_file)) {
					...
					if (function_exists('task') and mysql_query('START TRANSACTION') and ($task_result = task()) === true and mysql_query('COMMIT')) {
						...
						$this->set_next_runtime($task['results']['id']);
						$this->log_task($task['results']['id'], "Task successfully ran.");
						$this->unlock_task($task['results']['id']);
					} else {
						// The task ran unsuccessfully, so try to run it again in an hour, log its error, and unlock it.
						mysql_query('ROLLBACK');
						$this->try_again($task['results']['id']);
						$this->log_task($task['results']['id'], "Task error occurred: {$task_result}.", true);
						$this->unlock_task($task['results']['id']);
					}
				} else {
					// Log a "task not found" error.
					$this->log_task($task['results']['id'], "Task {$task_file} was not found.", true);
				}
			}
		}
	}
